    ...
    if (status == UM_DB_SUCCESS)
    {
        ...
        else
        {
                    ...
                    || (status == SQLITE_ROW))
            {
                ...
                {
                    idx = UtilGetColumnIndex(db, "data");
                    status = (SI_32)sqlite3_column_bytes(db->sql_statement, idx);
                    status = (status > (SI_32)buf_size)? (SI_32)buf_size : status;
                    memcpy(out_buf, sqlite3_column_blob (
                            db->sql_statement, idx), status);
                }
                else
                {
                    status = UM_DB_SUCCESS;
...
SI_32 UtilMngDelFromDb(t_util_db *db, const UI_8 *db_tablename,
        UI_8 *var_name)
{
    SI_32 status = UM_DB_FAILURE;
    SI_32 idx = (SI_32)0;
