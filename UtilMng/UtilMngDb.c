...
SI_32 UtilMngInitDb(t_util_db *db, const UI_8 *db_filename, t_util_db_callback callback)
{
    ...
    {
        sqlite3_snprintf(sizeof(db->db_filename), db->db_filename, "%s", db_filename);
        ...
        void *out_buf, UI_32 buf_size)
{
    ...
    status = UtilMngGetDataFromDb(db, db_tablename, NULL, out_buf, buf_size);

    if (status >= 0)
    {
        if (UtilMngDelDataFromDb(db, db_tablename, NULL) == UM_DB_FAILURE)
        {
            status = UM_DB_FAILURE;
        }
    }
